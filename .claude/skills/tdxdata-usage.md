...
tdxdata 是一个**用户自写的开源Python库**，采用类jqdata风格的函数式接口，封装了tdx-api的全部32个RESTful接口，提供实时行情、K线数据、分时数据等功能。
...
- **完全开源**: tdxdata Python库由用户自行开发，可自由使用、修改和分发
- **数据开源**: 数据来源于通达信公开服务器，属于免费开源数据
- **欢迎反馈**: 如果使用中发现任何问题或有改进建议，可联系开发者修改完善
- **自主可控**: 用户可自行修改代码以适应个性化需求

### 免责声明

- 本库仅供学习和研究使用
- 数据来源于通达信公共服务器，可能存在延迟
- 不构成任何投资建议，投资有风险
